...
# Initialize model once at module level to avoid race conditions
_model = None
...
def _init_model():
    ...
    """
    _model = genai.GenerativeModel('gemini-1.5-flash', system_instruction=system_instruction)
    return _model
...
class GreyService:
    ...
    async def chat(cls, message: str, context: dict) -> dict:
        ...
        try:
            # 3. Call LLM in a thread to avoid blocking the event loop
