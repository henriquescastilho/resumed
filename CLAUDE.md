...
The production iOS app built with **SwiftUI + Firebase + Core Data**. Open `Resumed/Resumed.xcodeproj` in Xcode.

- `Resumed/Resumed/` — Main app target
  - `Core/Models/` — Domain models: `User`, `Question`, `FlashCard`, `StudyPlan`
  - `Core/Services/` — Business logic: `AuthManager`, `APIClient`, `MockAPIClient`, `CoreDataManager`, `CoreDataStack`, `StoreKitManager`, `FocusSessionManager`, `ProgressTracker`, `ErrorReviewScheduler`, `SocialStatsManager`, `StudyPlanStore`, `FirebaseManager`, `NetworkMonitor`, `NotificationManager`
  ...
  - `Navigation/` — `AppState` (global state as `@ObservableObject`), `RootView` (auth/onboarding router), `TabBarView` (5 tabs: Home, Foco, GPS, ResuCard, Progresso)
- `ResumedTests/` — Unit/integration/E2E tests
- `ResumedWidget/` — iOS widget extension
- Requires `GoogleService-Info.plist` in the app target for Firebase
...
- **Auth flow:** Firebase Auth (Google Sign-In) → backend token exchange. Currently uses `MockAPIClient` for development (`APIClient.mode == .mock`).
...
- **Data persistence:** Core Data for local storage (questions, flashcards, study progress). `CoreDataStack` + `CoreDataManager` + entity extensions.
